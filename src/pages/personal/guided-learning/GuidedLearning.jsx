...
const GuidedLearning = () => {
  ...
  return (
      userDecision ? (
        ...
        <LearningDashboarb learning={learning} words={words} setUserDecision={setUserDecision}/>
  )
}

export default GuidedLearning

const LearningDashboarb = ({ learning, words, setUserDecision }) => {
  const [chunkPerc] = useState(Math.round(learning.words.reduce((acc, curr) => acc + curr.level.level, 0) * 100 / (learning.words.length * TARGET_PERFECT_LEVEL)))
  ...
  <div className='Guided-learn'>
    ...
    <div><Button className='continue-btn' variant="contained" disableElevation color='primary' onClick={() => setUserDecision(true)}>Continue</Button>
    </div>
  </div>
  )
}